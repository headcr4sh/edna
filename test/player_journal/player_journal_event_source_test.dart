...
import 'package:edna/player_journal/player_journal_event_record_source.dart';
...
void main() {
  group('PlayerJournalEventSource', () {
    ...
    test('.localPlayerJournalDefaultPath() returns a non empty String on Windows', () {
      if (Platform.isWindows) {
        final defaultPath = LocalPlayerJournalEventRecordSource.defaultPath();
        expect(defaultPath.isNotEmpty, true);
      }
    });
    test('.local(path: \$testdata).currentJournalFile() finds the latest journal file', () async {
      ...
      final currentJournalFile = await eventSource.currentJournalFile(refresh: true);
      expect(currentJournalFile != null, true);
      expect(path.basename(currentJournalFile!), 'Journal.210430110100.01.log');
    });
    test('.local(path: \$testdata).stream().first works as supposed to', () async {
      ...
      expect((await stream.first).event, 'Fileheader');
    });
  });
}
