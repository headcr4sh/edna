...
import 'package:flutter/material.dart' show BuildContext, ChangeNotifier, Locale, ThemeMode;
...
class EdnaAppSettings with ChangeNotifier {
  ...
  set mode(final EdnaAppMode mode) {
    _preferences.setString(_appModeKey, mode.name).then((_) => notifyListeners(), onError: (error, stackTrace) {
      logger.e('Error while saving settings.', error: error, stackTrace: stackTrace);
      return false;
    });
  }
