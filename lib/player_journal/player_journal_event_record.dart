...
import './player_journal_event_type.dart' show PlayerJournalEventType, PlayerJournalEventTypes;
...
abstract class PlayerJournalEventRecord {
  ...
  factory PlayerJournalEventRecord.fromJson(final Map<String, dynamic> json) {
    ...
    final PlayerJournalEventType event = json['event'];
    switch (event) {
      case PlayerJournalCommanderEventRecord.type:
        final String fid = json['fid'];
      ...
      case PlayerJournalFileheaderEventRecord.type:
        ...
        final String gameVersion = json['gameversion'];
        final String build = json['build'].trim();
      ...
      case PlayerJournalMusicEventRecord.type:
        final musicTrack = json['MusicTrack'];
      ...
      default:
        return PlayerJournalUnknownEventRecord(timestamp: timestamp);
    }
  }
  factory PlayerJournalEventRecord.fromString(final String jsonString) => PlayerJournalEventRecord.fromJson(jsonDecode(jsonString));
  @override
  String toString() {
...
class PlayerJournalCommanderEventRecord extends PlayerJournalEventRecord {
  ...
  : _fid = fid,
    _name = name,
    super(timestamp: timestamp, event: PlayerJournalCommanderEventRecord.type);
}
...
class PlayerJournalContinuedEventRecord extends PlayerJournalEventRecord {
  ...
  PlayerJournalContinuedEventRecord({
    ...
    required final int part,
      ...
      : _part = part,
        super(timestamp: timestamp, event: PlayerJournalContinuedEventRecord.type);
}
...
class PlayerJournalFileheaderEventRecord extends PlayerJournalEventRecord {
  ...
  : _part = part,
    ...
    _build = build,
    super(timestamp: timestamp, event: PlayerJournalFileheaderEventRecord.type);
}
...
class PlayerJournalMusicEventRecord extends PlayerJournalEventRecord {
  ...
  : _musicTrack = musicTrack,
    super(timestamp: timestamp, event: PlayerJournalMusicEventRecord.type);
}
...
class PlayerJournalShutdownEventRecord extends PlayerJournalEventRecord {
  ...
  PlayerJournalShutdownEventRecord({ required final DateTime timestamp }): super(timestamp: timestamp, event: PlayerJournalShutdownEventRecord.type);
}

class PlayerJournalUnknownEventRecord extends PlayerJournalEventRecord {
  static const type = '<<UNKNOWN>>';
  PlayerJournalUnknownEventRecord({ required final DateTime timestamp }): super(timestamp: timestamp, event: PlayerJournalUnknownEventRecord.type);
}
